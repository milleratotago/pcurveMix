...
pcm_MLSE <- function(ps, mu, sigma, pi, alpha, tails  # NEWJEFF: THIS IS NOT CORRECT
  ...
  if (thisrcond < pcm_env$small_rcond) {
    warning( paste0("Nearly singular information matrix (i.e., rcond <= ",pcm_env$small_rcond,")") )  # NEWJEFF: small_rcond no longer used?
